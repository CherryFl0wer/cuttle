...
/**
  ...
  * @author Depeng Liang
  * @src https://github.com/Ldpe2G/Graphviz4S/tree/master/src/main/scala/com/liangdp/graphviz4s/
  */
abstract class Dot(
  ...
  /**
    ...
    */
  def render(engine: String = "dot", format: String = "png", fileName: String,
             directory: String, cleanUp: Boolean = false): String = {
    val filePath = this.save(fileName, directory)
    val rendered = BackendDot.render(engine, format, filePath)
  ...
  def view(engine: String = "dot", format: String = "pdf", fileName: String,
           directory: String, cleanUp: Boolean = false): String = {
    this.render(engine, format, fileName, directory, cleanUp)
  }
